...
document.querySelectorAll(".column").forEach((e) => {
    e.addEventListener("click", () => {
        ...
        if (sum <= 6 && turn % 2 != 0) {
            document.getElementById(`${e.id}r${sum}`).style.backgroundColor = "palevioletred"
            turn++
            check('palevioletred')
            ...
            document.getElementById(`${e.id}r${sum}`).style.backgroundColor = "yellowgreen"
            turn++
            check('yellowgreen')
            document.getElementById("whosturn").innerText = "Pink's Turn"
